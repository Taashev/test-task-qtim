...
Подключиться к Postgres от имени суперпользователя и выполнить слудующие команды
...
Выдать пользователю права на БД qtim_db;

```sql
GRANT ALL PRIVILEGES ON DATABASE qtim_db TO qtim_user;
```

### Установка расширения для генерации UUID в Postgres
